...
//    caObjShip= new O3dObjPrim( O3dObjPrim::PrimCone, 5.0, 5, 5, 6.0 );
	...
	caObjShield = new O3dObjPrim( O3dObjPrim::PrimSphere, lSz*1.2, 10, 10 );
...
Pilot::scrollKamera()
{
  ...
  O3dKamera& lKam= *(WorldControler::GetCurrentWorld()->getKamera());
	...
	float lYThreshold = WorldGame::YSizeWorld*0.7;


	float lYLimit = 0.8;
	float lXLimitLeft  = -2;
	float lXLimitRight = 1;
...
//  cXDecalKamera = lD[0]*0.01;
 // cYDecalKamera = lD[1]*0.01;
   cXDecalKamera *= 0.05;
   cYDecalKamera *= 0.05;
 //std::cout << "x:" << lD[0]<< "\tdecal:" << cXDecalKamera  << std::endl;
